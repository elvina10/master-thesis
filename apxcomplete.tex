...
$\mathsf{pointsOrGadget}_{i, j}$ with 4 segments from $\mathsf{segmentsOrGadget}_{i,j}$.
\end{lemma}
...
points in $\mathsf{pointsClause}_i - \{a\}$ can be covered with 
a set of segments $P\_true^a_i$, a subset of $\segmentsClause_i$
...
For any $1 \le i \le n$,
points in $\mathsf{pointsClause}_i$ can be covered with
a set of segments $P\_false^a_i$, a subset of $\segmentsClause_i$
...
\begin{enumerate}[label={(\arabic*)}]
	...
	can not be covered using any subset of segments from $\segmentsClause_i$ of size less than 11;
	\item points in $\mathsf{pointsClause}_i$ can not be covered 
	using any subset of segments from $\segmentsClause_i$ of size less than 12.
\end{enumerate}
\end{lemma}
