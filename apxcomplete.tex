...
of the instance $(\points, \sets)$ of geometric set cover,
...
If $\sol$ contains both segments $\xTrueSegment$ and $\xFalseSegment$,
...
\begin{align}
	\begin{split}
	& \begin{cases}
	|\mathsf{pointsVar}_i \cap \sol| \ge 4 & \text{if}\ \xTrueSegment \in \sol \land \xFalseSegment \in \sol \\
	|\mathsf{pointsVar}_i \cap \sol| \ge 3 & \text{otherwise}
	\end{cases}
	\end{split}
\end{align}

If $\sol$ contains at most one of the segments $\xTrueSegment$ and $\xFalseSegment$,
...
the points $x_{i,0}, y_{i,0}, z_{i,0}$ were covered by
segments from $\sol \cap \segmentsVar_j$,
are not satisfied in $\phi$, but not all clauses
that cover one of these points with segment in $P_{var}^j$ are satisfied.

Let us look at such a $c_i$ and on points $x_{i,0}, y_{i,0}, z_{i,0}$
that are covered by $P_{var}^j$.
Consider the cases of choosing variable value in
equation \eqref{eqn:variable_assignment}.

If $\sol$ contains only one of the segments $\xTrueSegment$ and $\xFalseSegment$,
then the value $\phi(x_j)$ satisfies $c_i$.

If $\sol$ contains neither $\xTrueSegment$ nor $\xFalseSegment$,
