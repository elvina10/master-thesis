...
\item \label{part1} if there exists a solution to $\instanceGridTiling$, then there exists a solution
...
then there exists a solution to $\instanceGridTiling$.
\end{enumerate}
\end{lemma}


First, let us prove Theorem \ref{w1_hard} using Lemma \cite{w1_construction}.

\begin{proof}
Let us take any intance $\instanceGridTiling$ of grid tiling problem.
We prove this by contradiction, therefore we assume
...
If we could find the optimum solution of size at most $k$ to $I$,
let us assume it has weight $w$.
Using \ref{part2} we know that if $w \le \solWeight$,
then $\instanceGridTiling$ 
...
and \ref{part2} is proved by Lemma \ref{grid_tiling_exists}.
