...
$(1+\epsilon)$-approximation scheme even in the setting
with segments pararell to axis with 1/2-extensions.
Theorem \ref{segment_cover_apx_hard} implies
Theorem \ref{rectangle_cover_apx_hard}.
...
we would produce a general approximation scheme for MAX-(3,3)-SAT
(that doesn't exist).
...
segments, so you can assign variable a binary value.

Clause gadget is respoinsible for calculating if choice of the
variable values meets the clauses in formula.
It has minimal solution of $w$ if at least one variable
...
property for 2 variables (input segments).
There exists an output segment and minimal solution can
use the output segments only when at least one of the input
segments was chosen for a solution.

We also don't want the clause gadgets to be crammed inbetween
