...
TODO: expand the figure to include x, y and maybe name (t,v) as result

OR-gadget has 3 important segments
-- $x, y, result$, where $x$ and $y$ are inputs and part of different gadgets.

Gadget has a minimal solution of weight $w$
and $result$ can be part of minimal solution
only if $x$ or $y$ are also chosen
for the solution.
...
TODO: I don't think we need these, we can just show independent set
on CLAUSE-gadget and it's less confusing

\begin{lemma}
\label{cover_no_less_true}
For any $1 \le i \le n, j \in \{0, 1\}$ and 
 $x \in \{l_{i, j}, p_{i, j}\}$ points in
$\pointsOr{i}{j} - \{ x\} \cup \{v_{i, j}\}$
can not be covered with
with less than 4 segments from $\segmentsOr{i}{j}$.
\end{lemma}
...
For any $1 \le i \le n, j \in \{0, 1\}$, points in
$\pointsOr{i}{j}$ can not be covered
with less than 4 segments from $\segmentsOr{i}{j}$.
\end{lemma}
...
satisfies the respective clause in CNF.
It has minimal solution of weight $w$
the clause is satisfied, ie. at least one
...
how many clauses were possible to satisfy in the optimum solution of CNF.

The CLAUSE-gadgets consist of two OR-gadgets.
We do not want the CLAUSE-gadgets to be crammed 
somewhere between
the very long variable segments. That is why we have a simple
gadget to \textit{transfer} whether the segment
is in a solution, ie. segments
$(x_{i, 0}, x_{i, 1}), (y_{i, 0}, y_{i, 1}), (z_{i, 0}, z_{i, 1})$.
These consist of two segments
that share point $x_{i, 1}$
and these are the only segments that can cover $x_{i,1}$.
...
This emulates the situation as if $a$ was placed on the segment that
$x_{i,0}$ lays on, but uses one extra segment compared to
as $a = x_{i,0}$ and $x_{i, 1}$ did not existed.
...
TODO: maybe add segments choosing value of x, y, z to the figure with less opacity
