...
\begin{lemma}{
	...
	is a solution to the $\setCoverInstance$, then $\sol$
	is also a solution to the $\setCoverInstance$.
\end{enumerate}
...
describes proposed instance $\setCoverInstance$.
...
such that for any $i$ it contains at most
...
in $\sol$, we know that
$\phi(j)$ has the value that evaluates $q$ to be true.
