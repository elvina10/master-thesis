...
\begin{claim}
\label{guards}
In any solution of the instance $(\points, \sets, w)$:
\begin{itemize}
\item points in $\{p^L : p \in H\} \cup \{p^R : p \in H\}$
have to be covered with sgements from $\allhor$,
\item points in $\{p^U : p \in V\} \cup \{p^D : p \in V\}$
...
Every segment in $\allver$ is vertical and have different x-coordinate
than any $\{p^L : p \in H\} \cup \{p^R : p \in H\}$ points.

Every point $x$ from $\{p^L : p \in H\} \cup \{p^R : p \in H\}$
...
and any solution of the instance $(\points, \sets, w)$
...
For every solution of the instance $(\points, \sets, w)$
has to have sum of weights of segments chosen
