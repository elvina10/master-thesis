...
admits an FPT algorithm when parameterized with the size
...
there does not exist an FPT algorithm parameterized by solution size for it.
...
a \textit{dense} subset of points. Dense subset of points
...
In later part of the section Lemma \ref{dense_set_exists} will allow us to find a kernel
...
\begin{proof}[Proof of Lemma \ref{dense_set_exists}]
...
We name these segments $v_i$ and
...
Size of each $A_i$ is at most $(2+\frac{2}{\delta})^{k}$ from the inductive
