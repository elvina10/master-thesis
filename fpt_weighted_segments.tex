...
O(|C| + (1+\frac{1}{\delta})) + O(2^k(\sum_i^M |C_i|)k(1+\frac{1}{\delta}))
\le O(2^k|C|k(1+\frac{1}{\delta})).$$
...
covers at most one point that lays on $L$.

$L$ is long, so there are at least $k+1$ points from $\points$ that lay on $L$.
...
if we have more than $k$ long lines
