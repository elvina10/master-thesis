...
Let us assume that there exists a plynomial-time
$(1+\epsilon)$-approximation scheme
for unweighted geometric set cover with axis-parallel segments in 2D
with (1/2)-extensions.
We will construct an algorithm that solves the problem stated in 
...
From th defintion of $S$ being at most 
...
Lemmma \ref{apxconstruction} (3.), we have:
...
in a CNF formula. It allows two minimal solution
...
We denote the set of points marked with black circles as $C\_variable_i$,
...
We denote the set of red segments as $x^{false}_i$
and the set of green segments as $x^{true}_i$.}
