...
Let's recall some of the definitions for the approximation problems,
...
every $\epsilon > 0$,
that takes an instance $I$ of parametrization problem
...
that $(1+\epsilon)$-approximation scheme is NP-hard.
\end{defi}
...
$\delta$-extension of this segment, $L^{+\delta}$ is a segment
parallel to $L$, but both ends extended by $\delta d$,
ie. if we denote vector $t = (x_b - x_a, y_b - y_a)$,
$ L^{+\delta}
= \{ (x_a, y_a) - \delta\cdot t, (x_b, y_b) + \delta\cdot t \}$
\end{defi}

Relaxed cover problem with $\delta$-extensions allows
the objects to cover all the points in $\points$
after the $\delta$-extensions, and the found solution
doesn't have to be optimal for cover problem with $\delta$-extensions,
it must be not larger than
optimal solution for the problem without $\delta$-extensions.

Note that the found solution
for $\delta$-extensions can be smaller than optimal solution.
and there can be multiple correct solutions with different sizes
for cover problem with $\delta$-extensions.

\begin{defi}\textbf{Cover problem with $\delta$-extensions}
Optimization cover problem $\mathcal{P}$
that takes as an input instance $I$
and outputs a minimal solution $\mathcal{R}^{opt}$, such that
$\mathcal{R}$ covers $I$.

Optimization cover problem $\mathcal{P}$ with $\delta$-extensions,
takes as an input instance $I$
and output a solution $\mathcal{R}$, such that $\delta$-extended set
$\{ R^{+\delta} :  R \in \mathcal{R} \}$ covers $I$
and is no worse than minimal solution for this provlem without
extensions, ie. $|\mathcal{R}| \le |\mathcal{R}^{opt}|$.
\end{defi}
...
PTAS for cover problem $\mathcal{P}$ with $\delta$-extensions,
takes as an input instance $I$
and output a solution $\mathcal{R}$, such that $\delta$-extended set
$\{ R^{+\delta} :  R \in \mathcal{R} \}$ covers $I$
and is within $(1+\epsilon)$ optimization factor for
minimal solution for this provlem without
extensions, ie. $(1+\epsilon)|\mathcal{R}| \le |\mathcal{R}^{opt}|$.
