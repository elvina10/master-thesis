...
In this section we analyze if there exists $(1+\epsilon)$-approximation
scheme for set cover with rectangles.
We can restrict this problem to some very easy setting:
segments parallel to axes and allow (1/2)-extension
and the problem is still APX-hard.
Segments are just degenerated rectangles with one side being very narrow.
...
\label{segment_cover_apx_hard}
	...
	is APX-hard,
	ie. assuming $P\neq NP$, there doesn't exist a PTAS for this problem.
}\end{tw}
...
The following lemma encapsulated the properties of reduction described
in this section and it allows to prove the
...
\begin{lemma}{
	...
	there exists a solution of $S$ of size at most $15n - |X|$;
