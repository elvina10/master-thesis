...
To construct $\sol'$, 
we first remove all segments from $\segmentsVar{i}$ that we use.
...
assuming it is the $i$-th clause, then one of the points $x_{i,0}, y_{i,0}$ or $z_{i,0}$ for this
...
$x_{i,0}, y_{i,0}, z_{i,0}$ in $\pointsClause_i$ were covered by
segments from $\sol~\cap~\segmentsVar{j}$ for some $1 \le j \le n$,
then denote this point as $t$ and say it corresponds to
literal $w$ and variable $x_j$.
Point $t$ can be only covered by segments from $\segmentsClause{i}$
or a corresponding $\xTrueSegment{j}$ or $\xFalseSegment{j}$
(depending on whether the literal $w$ is negated or not).
We know that it is not covered by $\segmentsClause{i}$,
therefore it is covered by segments $\xTrueSegment{j}$ or $\xFalseSegment{j}$.
From definition of $\phi$ and the fact that one of these segments is
in $\sol$, we know that
