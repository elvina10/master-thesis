...
Therefore they have weight of segment $(h_{i,1,1}^L, h_{i,k,n^2}^R)$
minus $2\epsilon k$, which is $k(n^2+1) -2k\epsilon -2(1-\epsilon)$.
We need to multiply it by $k$, because this weight if equal for each $i$.
...
$$2k(k(n^2+1) -2k\epsilon -2(1-\epsilon)) + 2k^2\delta= \solWeight$$
\end{proof}
