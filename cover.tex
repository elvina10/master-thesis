...
We will find the kernel where at every line (not only segment)
there is not more than $k$ points.
...
There is at most one segment starting on each of the points
$x_1, x_2, \ldots x_{k+1}$,
so we have braching over $k+1$ choices on this segment.
