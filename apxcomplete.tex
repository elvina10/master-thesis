...
$\phi : \{ x_1, x_2 \ldots x_n\} \rightarrow \{\texttt{true}, \texttt{false}\}$.
...
let us name the variable that is \texttt{true} in it $x_i$
...
\begin{align}
	\begin{split}
	& R_i = \begin{cases}
		\chooseVar_i^{true} & \text{if}\ \phi(x_i) = \texttt{true} \\
		\chooseVar_i^{false} & \text{if}\ \phi(x_i) = \texttt{false} \\
		\end{cases} \\
	& C_i = \begin{cases}
...
then it have used at least 4 segments (Lemma~\ref{choose_variables_both}).
...
If $\sol$ contains none of these segments, set value to \texttt{false}.
...
\begin{align}
	...
	\phi(x_i) = true & \text{if}\ \xTrueSegment{i} \in \sol \\
...
we need to use at least 11 segments to cover $\pointsClause_i - \{x, y, z\}$
...
are not covered by the segments from $\pointsVar_i$,
then we need to cover $\pointsClause_i$
...
Consider clause, say $c_i$, if none of
...
If one of these points is covered by $\sol \cap \segmentsVar_j$,
say this point is $t$ and this point corresponds to variable $x_j$.
...
If we chose both $\xTrueSegment{j}$ and $\xFalseSegment{j}$,
then there are 3 clauses for which $t$
is covered by $\sol \cap \segmentsVar_j$.
We chose the variable value $\phi(x_j)$ in a way
...
11 segments from $\segmentsClause_i$, but are not satisfied by $\phi$.

This means that $\phi$ satisfies all but at most $a+b$ clauses in $S$.


To conclude, we proved that given a solution to $\setCoverInstance$ of size $w$ we have
constructed a variables assignment $\phi$ that satisfies at least $n-(a+b)$ clauses of $S$.
Finally, note that
...
By the Lemma~\ref{construction_correctness}, we know
