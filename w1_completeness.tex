...
\node[guard_v, label={left:$v_{i,j,1}^D$}] at (0, \l1) {};
\node[node_v, label={left:$v_{i,j,1}$}] at (0,\x1) {};
\node[guard_v, label={left:$v_{i,j,1}^U$}] at (0, \r1) {};
\node[guard_v, label={left:$v_{i,j,2}^D$}] at (0, \l2) {};
\node[node_v, label={left:$v_{i,j,2}$}] at (0,\x2) {};
\node[guard_v, label={left:$v_{i,j,2}^U$}] at (0, \r2) {};
\node[guard_v, label={left:$v_{i,j,3}^D$}] at (0, \l3) {};
\node[node_v, label={left:$v_{i,j,3}$}] at (0,\x3) {};
\node[guard_v, label={left:$v_{i,j,3}^U$}] at (0, \r3) {};
\filldraw[black] (0,\x4) circle (0pt) node[anchor=east] {$\ldots$};
\node[guard_v, label={left:$v_{i,j,n^2-1}^D$}] at (0, \l5) {};
\node[node_v, label={left:$v_{i,j,n^2-1}$}] at (0,\x5) {};
\node[guard_v, label={left:$v_{i,j,n^2-1}^U$}] at (0, \r5) {};
\node[guard_v, label={left:$v_{i,j,n^2}^D$}] at (0, \l6) {};
\node[node_v, label={left:$v_{i,j,n^2}$}] at (0,\x6) {};
\node[guard_v, label={left:$v_{i,j,n^2}^U$}] at (0, \r6) {};

\node[guard_h, label={left:$h_{i,j,1}^L$}] at (\l1,0) {};
\node[node_h, label={left:$h_{i,j,1}$}] at (\x1,0) {};
\node[guard_h, label={left:$h_{i,j,1}^R$}] at (\r1,0) {};
\node[guard_h, label={left:$h_{i,j,2}^L$}] at (\l2,0) {};
\node[node_h, label={left:$h_{i,j,2}$}] at (\x2,0) {};
\node[guard_h, label={left:$h_{i,j,2}^R$}] at (\r2,0) {};
\node[guard_h, label={left:$h_{i,j,3}^L$}] at (\l3,0) {};
\node[node_h, label={left:$h_{i,j,3}$}] at (\x3,0) {};
\node[guard_h, label={left:$h_{i,j,3}^R$}] at (\r3,0) {};
\filldraw[black] (\x4,0) circle (0pt) node[anchor=north] {$\ldots$};
\node[guard_h, label={left:$h_{i,j,n^2-1}^L$}] at (\l5,0) {};
\node[node_h, label={left:$h_{i,j,n^2-1}$}] at (\x5,0) {};
\node[guard_h, label={left:$h_{i,j,n^2-1}^R$}] at (\r5,0) {};
\node[guard_h, label={left:$h_{i,j,n^2}^L$}] at (\l6,0) {};
\node[node_h, label={left:$h_{i,j,n^2}$}] at (\x6,0) {};
\node[guard_h, label={left:$h_{i,j,n^2}^R$}] at (\r6,0) {};
...
\end{tikzpicture} 
\end{center}
\caption{\textbf{Vertices and segments from $\alldiag$ for fixed $1 \le i,j \le k$.}}
Nodes from $H$ and $V$ are marked in black, their guards are marked in blue.
You can see segments from $\alldiag$ with their weights (equal to $\delta$).
\label{fig:diag_def}
\end{figure}
