...
\begin{abstract}
	...
	but we compare the soltuion size to the optimum solution without extension.
	...
	(Section \label{section:fpt_weighted}).
	Finally we prove that $\WeightedSegmentSetCover$
	is W[1]-hard and there does not exist algorithm running
	in time $f(k)\cdot n^{o(\sqrt{k})}$ solving this problem
	even if we restrict segments to 3 directions
