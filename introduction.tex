...
That is, $L^{+\delta}$ is the the image of $L$ under homothety centred
at $S$ with scale $(1+\delta)$ without the edge.
Every segment is turned into a segment without endpoints
and any rectangle is turned into a interior of rectangle.
\end{defi}
...
results for $\SegmentSetCover$ and $\WeightedSegmentSetCover$.
...
\hline
                & weighted exact    & weighted $\delta$-extension & unweighted \\ 
\hline                
...
this problem is running in time ${f(k)(|\points| + |\sets|)^{O(k)}}$,
