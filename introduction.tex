...
\begin{defi}
\textbf{$\SegmentSetCover$} is a $\GeometricSetCover$, where
...
$\GeometricSetCover$ problems, where we allow the returned set of
...
We show that approximation of axis-parallel $\SegmentSetCover$
...
\label{segment_cover_apx_hard}
	\textbf{(Axis-parallel $\SegmentSetCover$ with $\frac{1}{2}$-extension is APX-hard)}.	
	Unweighted axis-parallel $\SegmentSetCover$
	in the 2D plane
...
of $\WeightedSegmentSetCover$ where segments are limited to 3 directions,
