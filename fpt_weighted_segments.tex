...
Assumption about the problem being relaxed with $\delta$-extensions is necessary.
In the next chapter we show that geometric set cover problem
for weighted segments is W[1]-hard, which means
...
of size bounded by some function $f(k, \delta)$ always exists.
In later part of the section Lemma \ref{good_set_exists} will allow us to find a kernel
...
\begin{lemma}
	...
	there exists a $(k,\delta)$-good set $A \subseteq C$ of size
	at most $(2+\frac{2}{\delta})^k$.
	Moreover there exists an algorithm that computes the $(k,\delta)$-good set
	in time $O(|C| \cdot (2+\frac{2}{\delta}))$.
\end{lemma}

To prove this theorem we define \textit{extreme points}
...
You can see an example of such segments $v_i$ and $t_i$ in Figure $\ref{fig:fpt_v_f_def}$.
...
$$M\left(2+\frac{2}{\delta}\right)^{k-1} =
\left\lceil1+\frac{2}{\delta}\right\rceil\cdot\left(2+\frac{2}{\delta}\right)^{k-1}
\le \left(2+\frac{2}{\delta}\right)^k$$
...
Both pictures show the same set $\points$ (black circles),
...
All segments from $\sol$ intersect with $t_4$.
...
$$\max(|y|, |z|)\delta \ge \frac{|s|\delta}{\delta+2} = \frac{|s|}{1+\frac{2}{\delta}} \ge \frac{|s|}{M} = v_i \ge t_i.$$
...
$$O\left(|C|+\frac{1}{\delta}\right) + O\left(k(2+\frac{1}{\delta})^k\right)$$
...
and that contradicts the fact that $\sol$ is a solution of the given
...
\item $C_i$, points that lie on the $i$-th long line.
\end{itemize}
