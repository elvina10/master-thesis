...
For the $i$-th CLAUSE-gadget we would like to place them between respective
$\xTrueSegment{j_1}$ or $\xFalseSegment{j_1}$ segments
corresponding to the literals that occur in the $i$-th clause.
It would be inconvenient to position them there, because between them,
there are already different parallel $\xTrueSegment{j_2}$ segments.
...
We place $x_{i,0}$ on a segment that we want to transfer (ie.
segment responsible for choosing the variable value satisfying the
corresponding literal).
If $x_{i,0}$ is already covered by this segment, then
...
Intuitively,
in optimal solution the two segments \textit{transfer} the state of whether $x_{i,0}$
is covered onto whether $a$ is covered.
Therefore with one additional segment it behaves as if $a$ was placed
on some segment $s$, but instead it can be placed anywhere on the plane,
...
To conclude,
given a solution of $\setCoverInstance$ of size $w$ we constructed
