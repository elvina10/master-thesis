...
Booleam formula is in \textbf{conjunctive normal form (CNF)} if
it is a conjunction of one or more fomulas,
which are disjuntion of literals.
...
$\mathcal{O}^{*}(2^{\sigma n})$.
Set us define $s_k$ as the infinimum  of the set $S_k$.
...
parametrized by the size of a solution $k$,
the task is to output a solution $\mathcal{R} \subseteq \sets$
such that the~$\delta$-extended set
$\{ R^{+\delta} :  R \in \mathcal{R} \}$ covers $\points$,
